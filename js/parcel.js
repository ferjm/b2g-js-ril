/*
   Parcel class for RIL transactions    
 */
function Parcel() {
...
with({parcel: Parcel.prototype}) {
  parcel.dataLeft = function() {
    return (Struct.CalcLength(this._buffer) - this._position);
  }

  parcel.setPosition = function(pos) {
    this._position = pos;
  }

  parcel.setBuffer = function(data) {
    this._buffer = data;
  }
  ...
  parcel.writeInt = function(i) {
    this._buffer = this._buffer.concat(Struct.Pack('i',[i]));
    this._format += 'i';
  }

  parcel.readInt = function() {
    var i = Struct.Unpack('i', this._buffer, this._position);
    this._position += 4;
    return i;
  }
